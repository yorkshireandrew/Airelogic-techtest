...
## SOLID Principles
  ...
  - Check the code for performance, maintainability and scalability concerns such as inefficient algorithms, unnecessary computations, or potential bottlenecks. mention concerns with a "WARNING: [DESCRIPTION OF ISSUE]" note in your final response.

## Security
  Always perform the following security checks on any code you write or modify:
  - Check code for SQL injection. Parameterized queries good. constructed SQL queries bad. Mention any concerns with a "SQL INJECTION: [DESCRIPTION OF ISSUE]" note in your final response.
  - Check code for HTML injection, ensure all text that may have been provided by a user is properly escaped. Mention any concerns with a "HTML INJECTION: [DESCRIPTION OF ISSUE]" note in your final response.
  - Check code for potential information disclosure, ensure sensitive data such as secrets, keys, personally identifiable information or internal system details are not exposed in logs, error messages, or API responses. Mention any concerns with an "INFO DISCLOSURE: [DESCRIPTION OF ISSUE]" note in your final response.
  - Check code for Missing/weak authorization checks at entry points, Client-side injection (XSS, DOM XSS), CSP issues, hardcoded secrets, Insecure deserialization, Insecure direct object references. Mention any concerns with a "SECURITY: [DESCRIPTION OF ISSUE]" note in your final response.
