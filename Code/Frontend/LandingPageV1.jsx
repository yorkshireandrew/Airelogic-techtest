...
export default function LandingPage({ visible = true }) {
  ...
  const handleSubmit = (e) => {
    ...
    // Surname required after trimming
      ...
      <form id="health-form" ref={formRef} method="post" action="/landing-submit" onSubmit={handleSubmit}>
        <div>
          ...
          <input
            ...
            title="Enter a 9- or 10-digit NHS number"
            ref={nhsRef}
            onInput={() => nhsRef.current && nhsRef.current.setCustomValidity('')}
          />
        </div>
